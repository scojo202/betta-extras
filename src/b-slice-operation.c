                                     ...
                                     BData * input)
{
  ...
  if (B_IS_VECTOR(input)) {
    ...
    d->input =
      b_create_input_array_from_vector(vec, neu, d->size.columns, d->input);
...
static void b_slice_operation_class_init(BSliceOperationClass * slice_klass)
{
  ...
  g_object_class_install_property(gobject_klass, SLICE_PROP_INDEX,
      g_param_spec_int("index", "Index", "Index of slice",
                        0, 2000000000, 0,
                        G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

  g_object_class_install_property(gobject_klass, SLICE_PROP_TYPE,
      g_param_spec_int("type", "Type", "Type of slicing operation",
                        SLICE_ROW, 7, SLICE_ROW,
                        G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

  g_object_class_install_property(gobject_klass, SLICE_PROP_WIDTH,
      g_param_spec_int("width", "Width", "Width of slice, if appropriate",
                        -1, 2000000000, 1,
                        G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

  g_object_class_install_property(gobject_klass, SLICE_PROP_MEAN,
